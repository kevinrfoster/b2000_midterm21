# data from Household Pulse Week 36
...
d1 <- read.csv("pulse2021_puf_36.csv")
...
d2 <- d1[c("RHISPANIC","RRACE","EEDUC","MS","EGENID_BIRTH","GENID_DESCRIBE","SEXUAL_ORIENTATION",
           ...
           "RECVDVACC","DOSESRV","GETVACRV","KIDDOSES","KIDGETVAC",
           "HADCOVID","WRKLOSSRV","ANYWORK","KINDWORK","RSNNOWRKRV",
           "CHLDCARE","ACTIVITY1","ACTIVITY2","ACTIVITY3","ACTIVITY4",
...
levels(d2$CHLDCARE) <- c("NA","NA","yes impacts to childcare because pandemic","no")
...
levels(d2$FORCLOSE) <- c("NA","NA","very likely forclosed in next 2 months","somewhat likely forclosed in next 2 months","not very likely forclosed in next 2 months","not at all forclosed evicted in next 2 months")
...
set.seed(12345)
sel1 <- (runif(dim(d2)[1]) > 0.1)

d3 <- subset(d2,sel1)

xtabs(~d3$EEDUC + d3$RECVDVACC + d3$GENID_DESCRIBE)
xtabs(~d3$EEDUC + d3$RECVDVACC + d3$PRIVHLTH + d2$PUBHLTH)
xtabs(~d3$EEDUC + d3$RECVDVACC + d3$REGION)
...
Household_Pulse_data <- Household_Pulse_data[ , !names(Household_Pulse_data) %in% c("TENROLLPUB","TENROLLPRV","TENROLLHMSCH","ACTIVITY1","ACTIVITY2","ACTIVITY3","ACTIVITY4")]

save(Household_Pulse_data, file = "Household_Pulse_data.RData")
